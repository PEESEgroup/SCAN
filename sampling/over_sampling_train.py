...
def smoter_focus_high_y(X_list, y, k_neighbors=5, n_samples=500, y_threshold=10):
    rare_indices = np.where(y > y_threshold)[0]
    X_rare_list = [X[rare_indices] for X in X_list]
    y_rare = y[rare_indices]

    knn = NearestNeighbors(n_neighbors=k_neighbors).fit(np.expand_dims(y_rare, axis=1))
    neighbors = knn.kneighbors(np.expand_dims(y_rare, axis=1), return_distance=False)

    X_new_list = [[] for _ in range(len(X_list))]
    y_new = []

  
    for _ in range(n_samples):
        idx = np.random.choice(len(y_rare))
        neighbor_idx = np.random.choice(neighbors[idx])

       
        alpha = np.random.rand()
        for i, X_rare in enumerate(X_rare_list):
            x1, x2 = X_rare[idx], X_rare[neighbor_idx]
            X_new_list[i].append(alpha * x1 + (1 - alpha) * x2)

        y1, y2 = y_rare[idx], y_rare[neighbor_idx]
        y_new.append(alpha * y1 + (1 - alpha) * y2)


    X_resampled_list = [
        np.vstack([X, np.array(X_new)]) for X, X_new in zip(X_list, X_new_list)
    ]
...
def train_model(train_data, train_labels, val_data, val_labels, epochs=1000, type=False):

    model = create_model().to(device)
    optimizer = create_optimizer(model)
    criterion = create_criterion()
    
    train_data_balanced, train_labels_balanced = smoter_focus_high_y(train_data, train_labels, k_neighbors=5, n_samples=your number, y_threshold=your cutoff)
...
A_feature_1 = np.load("../../salt_features.npy") # 14
A_feature_2 = np.load("../../solvent_features.npy") # 14
A_feature_3 = np.load("../../condition_features.npy") # 6
